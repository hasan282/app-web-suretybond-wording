...
class BUMIDA_APB2_NKC extends BaseWording
{
    ...
    public function content()
    {
        ...
        $this->setPoint('Bahwa <b>' . $this->data('principal') . '</b> dengan suatu perjanjian tertulis <bi> No. 006/YMCI/CON/VII/14 </bi> tanggal <b>' . $this->data('date_from') . '</b> telah mengadakan kontrak dengan <b>OBLIGEE</b> untuk pekerjaan <bi>' . $this->data('proyek_nama') . '</bi> dengan Harga Kontrak yang telah disetujui sebesar <bi>' . $this->data('symbol') . ' ' . nformat($this->data('nilai')) . '</bi> <bi>(' . $this->terbilang('nilai') . ' ' . $this->data('currency') . ')</bi> dan kontrak tersebut merupakan bagian yang tidak terpisahkan dari jaminan ini.');
        $this->setPoint('Bahwa untuk Kontrak tersebut diatas, <b>' . $this->data('obligee') . '</b> setuju membayar kepada <b>PRINCIPAL</b> uang sebesar <bi>' . $this->data('symbol') . ' ' . nformat($this->data('nilai')) . '</bi><bi>(' . $this->terbilang('nilai') . ' ' . $this->data('currency') . ')</bi> sebagai pembayaran uang muka sebelum Pekerjaan menurut Kontrak diatas dimulai. Sebagai jaminan terhadap pembayaran Uang Muka itu <b>SURETY</b> memberikan jaminan dengan ketentuan tersebut dibawah ini.');
        $this->setPoint('Jika <b>PRINCIPAL</b> telah melakukan pembayaran kembali kepada <b>OBLIGEE</b> seluruh jumlah Uang Muka dimaksud (yang dinyatakan dalam surat tanda bukti penerimaan olehnya) atau sisa Uang Muka yang wajib dibayar menurut Kontrak tersebut maka surat Jaminan ini menjadi batal dan tidak berlaku; sebaliknya jika tidak maka Surat Jaminan ini tetap berlaku dan efektif mulai dari tanggal <b>' . $this->data('date_from') . ' sampai dengan ' . $this->data('date_to') . '</b>(selama berlakunya Kontrak atau sampai pada tanggal Uang Muka telah dibayar kembali seluruhnya)');
        ...
        $this->setPoint('<b>SURETY</b> akan membayar kepada <b>' . $this->data('obligee') . '</b> Uang Muka atau sisa Uang Muka yang berdasarkan kontrak belum dikembalikan oleh <b>PRINCIPAL</b> setelah menerima tuntutan penagihan (Klaim) dari <b>OBLIGEE</b>.');
        ...
        $this->setPoint('Ditandatangani serta dibubuhi materai di ' . $this->data('issued_place') . ' pada tanggal <b>' . $this->data('issued_date') . '</b> PRINCIPAL <b>' . $this->data('principal') . '</b> SURETY <b>' . $this->data('asuransi_print') . '</b>');
        $this->setContent();
        return $this;
    }
}
